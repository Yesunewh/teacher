		...
		<div class="row">
			...
			@foreach($list as $item)
				...
				<div data-filter="{{$item->filters}}" class="col-lg-4 col-xl-3 col-md-6 pt-8 pb-10 px-16 relative border-[1px] border-solid border-t-0 border-s-0 border-[var(--tblr-border-color)] group max-xl:px-10">
					<span class="text-white avatar w-full h-[43px] mb-[18px] [&_svg]:w-[20px] [&_svg]:h-[20px] relative transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg rounded-md overflow-hidden" style="background: {{$item->color}}">
						@if ($item->image !== 'none')
						  <span class="inline-block  text-white transition-all duration-300 group-hover:scale-140">
					...
					@if($item->active == 1)
					@if (!$upgrade)
						<a onclick="return favoriteTemplate({{$item->id}});" id="favorite_area_{{$item->id}}" class="btn inline-flex items-center justify-center w-[34px] h-[34px] p-0 absolute  right-4 z-3">
