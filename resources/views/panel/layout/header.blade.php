		...
		<div class="p-0 container-fluid max-lg:w-full">
			<h1
				...
				<a class="block px-0" href="{{ LaravelLocalization::localizeUrl( route('dashboard.index') ) }}">
					...
					<img src="/{{$setting->logo_dashboard_path}}" @if ( isset($setting->logo_dashboard_2x_path) )
					srcset="/{{$setting->logo_dashboard_2x_path}} 2x" @endif alt="{{$setting->site_name}}" class="w-full
					h-auto group-[.navbar-shrinked]/body:hidden dark:hidden">
					<img src="/{{$setting->logo_dashboard_dark_path}}" @if (
						isset($setting->logo_dashboard_dark_2x_path) )
					srcset="/{{$setting->logo_dashboard_dark_2x_path}} 2x" @endif alt="{{$setting->site_name}}"
					class="w-full h-auto hidden group-[.navbar-shrinked]/body:hidden dark:block">
					@else
					<img src="/{{$setting->logo_path}}" @if ( isset($setting->logo_2x_path) )
					srcset="/{{$setting->logo_2x_path}} 2x" @endif alt="{{$setting->site_name}}" class="w-full h-auto
					group-[.navbar-shrinked]/body:hidden dark:hidden">
					<img src="/{{$setting->logo_dark_path}}" @if ( isset($setting->logo_dark_2x_path) )
					srcset="/{{$setting->logo_dark_2x_path}} 2x" @endif alt="{{$setting->site_name}}" class="w-full
					h-auto hidden group-[.navbar-shrinked]/body:hidden dark:block">
					@endif

					<!-- collapsed -->
					<img src="/{{$setting->logo_collapsed_path}}" @if ( isset($setting->logo_collapsed_2x_path) )
					srcset="/{{$setting->logo_collapsed_2x_path}} 2x" @endif alt="{{$setting->site_name}}" class="w-full
					h-auto hidden max-w-[40px] mx-auto group-[.navbar-shrinked]/body:block dark:!hidden">
					<img src="/{{$setting->logo_collapsed_dark_path}}" @if (
						isset($setting->logo_collapsed_dark_2x_path) )
					srcset="/{{$setting->logo_collapsed_dark_2x_path}} 2x" @endif alt="{{$setting->site_name}}"
			...
			<div class="collapse navbar-collapse" id="navbar-menu">
				<ul class="primary-nav navbar-nav max-lg:py-4 max-lg:px-3">
					...
					<li class="nav-item">
						<a class="nav-link {{activeRoute('dashboard.teacher.curriculum')}}"
							href="{{route('dashboard.teacher.curriculum')}}">
							<span class="nav-link-icon">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
					...
					{{-- <li class="nav-item">
						<a class="nav-link {{activeRoute('dashboard.teacher.assessment')}}"
							href="{{route('dashboard.teacher.assessment')}}">
							<span class="nav-link-icon">
								<span class="nav-link-icon">
									<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
										stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
						...
						<a class="nav-link {{activeRoute('dashboard.teacher.curriculum')}}"
							...
							<span class="nav-link-icon">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" stroke-width="1.5"
									fill="none" stroke-linecap="round" stroke-linejoin="round" class="bi bi-layers"
									viewBox="0 0 24 24">
									<path
					...
					<li class="nav-item">
						<a class="nav-link {{ route('dashboard.user.openai.chat.list') == url()->current() ? 'active' : '' }}"
							href="/dashboard/user/openai/chat/ai-chat/ai-chat-bot">
							<span class="nav-link-icon">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
									stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
					...
					{{-- <li class="nav-item">
						<a class="nav-link {{activeRoute('dashboard.user.openai.list')}}"
							href="{{route('dashboard.teacher.other')}}">
							<span class="nav-link-icon">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
									stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
									<path d="M4 4h6v6h-6z"></path>
					...
					{{-- <li class="nav-item">
						<a class="nav-link {{activeRoute('dashboard.admin.advertis.index')}}"
							href="{{route('dashboard.admin.advertis.index')}}">
							<span class="nav-link-icon">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
									stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
									<path d="M12 12m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0"></path>
					...
					<li>
						<a class="nav-link {{activeRoute('dashboard.teacher.managestudent')}}"
							href="{{route('dashboard.teacher.managestudent')}}">
							<span
								...
								{{mb_substr(__('Manage Student'), 0, 1)}}
							</span>
							<span class="flex items-center transition-[opacity,transform] nav-link-title grow">
								{{__('Manage Student')}}
							</span>
					...
					<li>
						<a class="nav-link {{activeRoute('dashboard.teacher.students')}}"
							href="{{route('dashboard.teacher.students')}}">
							<span
								class="nav-link-icon inline-flex items-center justify-center w-[24px] h-[24px] shrink-0 rounded-[6px] bg-[#7A8193] text-white text-[10px] font-normal">
								{{mb_substr(__('List Student'), 0, 1)}}
							</span>
							<span class="flex items-center transition-[opacity,transform] nav-link-title grow">
								{{__('Student List')}}
							</span>
					...
					<li class="nav-item">
						<a class="nav-link {{activeRoute('dashboard.admin.ads.index')}}"
							...
							<span
								class="nav-link-icon inline-flex m-1 items-center border border-dark p-0 justify-center stroke-width='1.5' w-[15px] h-[15px] shrink-0 rounded-[6px] bg-[#ffffff] text-dark text-[13px] font-normal">
						...
						<div class="nav-link-label transition-all">
							<div
								...
								<p class="mb-[0.75em]">{{__('Invite your friend and get')}}
									{{$setting->affiliate_commission_percentage}}% {{__('on all their purchases.')}}</p>
								<p class="m-0">
					...
					<div class="dropdown-menu dropdown-menu-end">
						<div class="flex flex-col">
							...
							<a rel="alternate" hreflang="{{ $localeCode }}"
								...
								<span class="text-[21px] !me-2">{{ country2flag(substr($properties['regional'],
									strrpos($properties['regional'], '_') + 1)) }} </span>{{$properties['native']}}
							</a>
							@endif
							@endforeach
						</div>
								...
								<div class="row">
									<div class="col-auto d-flex align-items-center">
										...
										<span class="d-none d-md-inline d-lg-none d-xxl-inline ms-2 text-muted">{{
											(int)Auth::user()->remaining_words != -1 ?
											number_format((int)Auth::user()->remaining_words) : __('Unlimited')
											}}</span>
									</div>
									<div class="col-auto d-flex align-items-center">
										<span class="legend !me-2 rounded-full bg-[#9E9EFF]"></span>
										...
										<span class="d-none d-md-inline d-lg-none d-xxl-inline ms-2 text-muted">{{
											(int)Auth::user()->remaining_images != -1 ?
											number_format((int)Auth::user()->remaining_images) : __('Unlimited')
											}}</span>
									</div>
								</div>
							</li>
	...
	<ul class="grid grid-cols-4 place-items-center w-full p-0 m-0 list-none">
		...
		<li class="w-full">
			<button
				...
				type="button" data-bs-toggle="collapse" data-bs-target="#navbar-search" aria-controls="navbar-search"
	...
	<ul class="p-0 m-0 h-full relative list-none text-[13px] text-heading font-medium">
		...
		<li class="relative group">
			<a class="flex items-center gap-2 p-3 py-2 border-solid border-b border-r-0 border-t-0 border-l-0 border-[--tblr-border-color] text-inherit"
				...
				href="{{ LaravelLocalization::localizeUrl( route('dashboard.user.openai.generator.workbook',
				$aiWriter->slug)) }}"
...
<div class="fixed bottom-20 end-12 z-50 hidden group lg:block">
	<button
		...
		type="button">
		<span
			class="inline-flex items-center justify-center w-full h-full mb-1 relative rounded-full overflow-hidden before:w-full before:h-full before:absolute before:top-0 before:left-0 before:rounded-full spinner_button before:animate-spin-grow">
