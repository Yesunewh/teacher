						...
						<div class="container-tight">
							...
							<form novalidate="novalidate" onsubmit="return RegisterForm();">
								...
								<div class="mb-[20px]">
									...
									<select id="woreda" name="woreda_id" class="form-control">
...
<script>
    $(document).ready(function() {
        ...
        $('#region').on('change', function() {
            ...
            // Clear the zone and woreda select inputs
            $('#zone').empty().append('<option value="">Select Zone</option>');
            $('#woreda').empty().append('<option value="">Select Woreda</option>');

            if (region_id) {
                // Make an AJAX request to fetch the zones based on the selected region
                $.ajax({
                    url: '/zones/' + region_id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        // Populate the zone select input with the retrieved zones
        ...
        $('#zone').on('change', function() {
            ...
            // Clear the woreda select input
            $('#woreda').empty().append('<option value="">Select Woreda</option>');

            if (zone_id) {
                // Make an AJAX request to fetch the woredas based on the selected zone
                $.ajax({
                    url: '/woredas/' + zone_id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        // Populate the woreda select input with the retrieved woredas
                        $.each(data, function(key, woreda) {
		...
		$('#woreda').on('change', function() {
        var woreda_id = $(this).val();

        // Clear the school select input
        $('#school').empty().append('<option value="">Select School</option>');

        if (woreda_id) {
            // Make an AJAX request to fetch the schools based on the selected woreda
            $.ajax({
                url: '/schools/' + woreda_id,
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    // Populate the school select input with the retrieved schools
