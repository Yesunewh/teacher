...
class AIController extends Controller
{
    ...
    public function buildOutput(Request $request)
    {
        ...
        //Unit Planner
        if ($post_type == 'unit_planner') {

            $title = $request->title;
            $focus_keywords = $request->title;
            $grade = $request->grade;
            $Subject = $request->Subject;
            $prompt = "Design a unit plan for  $grade students focusing on $title subject $Subject .  Include  lessons with clear objectives and engaging activities. Assess student understanding throughout the unit. Use diverse resources and provide a structured outline. Maximum $maximum_length words. Creativity is $creativity between 0 and 1. Language is $language. Generate $number_of_results unique unit plans with Tone of voice must be $tone_of_voice";
        }

        if ($post_type == 'syllabus-generator-G89aJy') {
            ...
            // $prompt = "Design a syllabus for  $grade students focusing on $title subject $Subject .  Include  lessons with clear objectives and engaging activities. Assess student understanding throughout the unit. Use diverse resources and provide a structured outline. Maximum $maximum_length words. Creativity is $creativity between 0 and 1. Language is $language. Generate $number_of_results unique unit plans with Tone of voice must be $tone_of_voice";
            $prompt = "Design a syllabus for Grade $grade students in the subject of $subject, titled '$title'.  The syllabus should include:  Description: $description Objectives: $objectives     Required Materials: $materials   Please create a comprehensive syllabus that engages students through interactive activities and provides them with the necessary resources to understand and apply vector concepts effectively.  Maximum $maximum_length words. Creativity is $creativity between 0 and 1. Language is $language. Generate $number_of_results unique unit plans with Tone of voice must be $tone_of_voice";
        }
        ...
        for ($i = 0; $i < $number_of_images; $i++) {
            if ($image_generator != self::STABLEDIFFUSION) {
                ...
                if ($response->getStatusCode() == 200) {
                    ...
                    $nameprompt = substr($prompt[0]['text'], 0, 15);
                    $nameprompt = explode(' ', $nameprompt)[0];
